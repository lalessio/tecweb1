	...
	\newpage
	 ...
	 Sono stati inseriti per ogni pagina i tag meta:
			...
			\begin{itemize}
				...
				\item Inoltre, in tutte le pagine, \`e stato inserito un link skip nav per saltare direttamente al contenuto qualora l'utente dello screen reader non voglia riascoltare nuovamente il men\`u
				
			\end{itemize}
\newpage			\section{Usabilit\`a}
			L'utente riesce ad orientarsi nel sito? Analizziamo alcuni degli assi principali del giornalismo.
			\begin{itemize}
				
				\item What?: 
				Un utente appena entra nella home capisce subito che si tratta di un sito di un parco naturale, dall'immagine e dal logo del parco, e dal contenuto in primo piano in cui vengono messe in evidenza (attraverso box con immagine e link) le principali sezioni del sito(Chi siamo,Natura,Orari e prezzi,Info e contatti).
				
				\item Who?:
				L'asse who viene descritto nella pagina Chi siamo,
				in cui viene descritta la storia del parco dagli inizio fino ad oggi.La pagina Chi siamo \'e accessibile dal men\'u.
				...
				\item Why?: 
				Perch\'e un utente dovrebbe rimanere nel sito o dovrebbe ritornarci? Il sito \'e principalmente espositivo,(gli utenti possono liberamente visualizzare le news e novit\'a riguardanti il parco),
				gli orari e i prezzi dei biglietti per entrare nel parco. 
				Solo da lato amministratore \'e possibile inserire nuove notizie, modificare i prezzi e gli orari.
